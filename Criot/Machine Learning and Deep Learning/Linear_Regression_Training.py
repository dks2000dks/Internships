...
df = pd.read_csv('Intensity_Regression_Data.csv',sep=',')															#Importing the .csv for data analysis
...
def preprocess_targets(df):
	...
	output_targets["Read_Mode"] = df["Read_Mode"]
	
	return output_targets


training_examples = preprocess_features(df.head(56))										#First 23 of the data will be for Training
print (training_examples.describe())
print ("--------------------------------------------------------------------------")

training_targets = preprocess_targets(df.head(56))
...
def train_model(learning_rate,steps,batch_size,training_examples,training_targets,validation_examples,validation_targets):
	...
	training_input_fn = lambda: my_input_fn(training_examples,training_targets["Read_Mode"],batch_size=batch_size)
	predict_training_input_fn = lambda: my_input_fn(training_examples,training_targets["Read_Mode"],num_epochs=1,shuffle=False)
	predict_validation_input_fn = lambda: my_input_fn(validation_examples, validation_targets["Read_Mode"],num_epochs=1,shuffle=False)
	...
	for period in range (0, periods):
		...
		weight = Linear_Regressor.get_variable_value('linear/linear_model/DayLight/weights')[0]
...
Linear_Regressor = train_model(learning_rate=0.05,steps=1000,batch_size=10,training_examples=training_examples,training_targets=training_targets,validation_examples=validation_examples,validation_targets=validation_targets)
print ("--------------------------------------------------------------------------")
...
plt.scatter(df["DayLight"],df["Read_Mode"])
